...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    ...
    assert (output.dim() in [1, 2, 3]),output.shape
...
#     TP, TN, FP, FN = 0, 0, 0, 0
    ...
    with torch.no_grad():
        ...
        intersection, union, new_target = intersectionAndUnionGPU(y_pred[:,0,:,:], y_true[:,0,:,:], K=2)
        intersection, union, new_target = intersection.cpu().numpy(), union.cpu().numpy(), new_target.cpu().numpy()
        
        y_true = y_true.cpu().numpy()
        y_pred = y_pred.cpu().numpy()
        TP, TN, FP, FN ,_,_ = get_binary_classification_metrics(y_pred, y_true, threshold=0.5)

    return intersection, union, new_target,TP, TN, FP, FN
